...
<script>
    ...
    function initMap() {
        ...
        for (let i = 0; i < markerData.length; i++) {   
            ...
            google.maps.event.addListener('spider_format', function(marker, status) {
                var iconURL = 
                    status == OverlappingMarkerSpiderfier.markerStatus.SPIDERFIABLE ? "{{asset('assets/img/marker-plus.svg')}}" :
                    status == OverlappingMarkerSpiderfier.markerStatus.UNSPIDERFIABLE ? "{{asset('storage/')}}"+ "/" + markerData[i].url + '#custom_marker':
                    null;
                marker.setIcon({
                    url: iconURL,
